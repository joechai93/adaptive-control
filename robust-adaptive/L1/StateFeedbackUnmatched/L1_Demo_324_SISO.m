...
for i=3:length(t)
    ...
    % control law
    eta_1 = theta1_est(i-1)*norm(x(:,i-1),'Inf') + sigma1_est(i-1);
    eta_2 = theta2_est(i-1)*norm(x(:,i-1),'Inf') + sigma2_est(i-1);
    eta_2m = Hm_i * C_Hu * x_unmatched(:,i-1);
    
    %Hm = 
    
    u_raw(i-1) = (eta_1 + eta_2m - kg*r)/omega_est(i-1);
    ...
    omega_estdot  = gam * Proj(omega_est(:,i-1), -(xtilde'* P * Bm)' * u(:,i-1)', omega_max);
    
    % compute relevant dynamics
    xdot_pred      = Am*x_pred(:,i-1) ...
                     + Bm*(theta1_est(i-1)*norm(x(:,i-1),'Inf') + omega_est(i-1)*u(i-1) + sigma1_est(i-1)) ...
